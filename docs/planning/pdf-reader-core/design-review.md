...
| C2 | T1 interface exposes only AC-required commands; may evolve through T7; not frozen until close. | Avoid designing a boundary's details before its shape is known, without losing the parallelizing seam. | **Done** — `reader.Reader` exposes exactly Open / PageCount / RenderPage / Thumbnail / Get+SetPosition / Close; no speculative commands. Still open to change through T7. |
| C3 | Reading-position store stays a narrow `Save`/`Load` + minimal record; no general repository layer. Swappable for SQLite (ADR-0008) with no AC change. | Justified seam, but guard against it growing into speculative generality. | **Done** — `library.Store` is exactly `Load`/`Save` over one `Record`; `FileStore` is the interim impl, `NewEngineWithStore` is the swap point for SQLite. No repository abstraction. |
